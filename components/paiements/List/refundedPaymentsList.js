...
import { GET_ALL_REFUNDED_PAYMENTS } from "../../../queries/paiements/getRefundedPayments"
...
const RefundedPaymentsList = () => {
    const {range, setRange} = useContext(DateContext)
    const [GetAllRefundedPayments, { loading, error, data, refetch }] = useLazyQuery(GET_ALL_REFUNDED_PAYMENTS)
    ...
    if(!data.getRefundedTransactionByEnterpriseId.length) return <EmptyPanel title={"Aucune transaction"} description={"Vous n'avez pour l'heure recu aucun paiement ou transaction"}/>

    return (
       
        <Transition show={data.getRefundedTransactionByEnterpriseId.length > 0}>
                    ...
                    <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {
                                    data.getRefundedTransactionByEnterpriseId.map((paiement, key) => {
...
export default RefundedPaymentsList