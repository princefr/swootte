...
import { GET_ALL_SUCCESFULL_PAIEMENT } from "../../../queries/paiements/getSuccessfullPayments"
...
const SuccessPaymentsList = () => {
    ...
    const [GetSuccessFullPaiment, {loading, error, data, refetch }] = useLazyQuery(GET_ALL_SUCCESFULL_PAIEMENT)
    ...
    if(!data.getSuccessFullTransactionByEnterpriseId.length) return <EmptyPanel title={"Aucune transaction"} description={"Vous n'avez pour l'heure recu aucun paiement ou transaction"}/>

    return (
        <Transition show={data.getSuccessFullTransactionByEnterpriseId.length > 0}>
                    ...
                    <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {
                                    data.getSuccessFullTransactionByEnterpriseId.map((paiement, key) => {
...
export default SuccessPaymentsList